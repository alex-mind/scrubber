...
[the latest release](https://github.com/alex-mind/scrubber/releases/latest), or build it:

```sh
./build.sh chrome     # -> build/extension/ and dist/scrubber-<version>.zip
```

Then in Chrome, Edge, Brave, Arc or Opera:
...
Safari needs a $99/yr Apple developer account to *ship*, so it isn't on a store.
Building it locally is free — see [notes/safari.md](notes/safari.md).
