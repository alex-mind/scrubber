...
Why the code is shaped the way it is. Most entries exist because the obvious
implementation was tried first and was wrong — the note records the failure so the
bug does not get reintroduced as a "simplification".

**If you are an agent changing this code: read the entries touching your area before
you edit.** Several of these look like dead weight until you know what they prevent.
